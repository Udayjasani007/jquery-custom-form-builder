$(document).ready(function () {
    ...
    // focus out setting and delete button hide
    ...
    // focus in setting and delete button display
   ...
   // right side menu open
    ...
    // open right side menu and give ids accordingly
    $(document).on('click', ".add_elements", function () {
        ...
        console.log(tab_id_input, side_nav_id);
    ...
    // changes all fields to drop-down
    ...
    // changes rows according to tables
    ...
    // changes select button on right side nav according to tables
...
$(document).on('click', '.submit', function () {
    ...
    let len = $('.table-drop .forms-sect').length;
    console.log(len);
    for (let i = 0; i < len; i++) {
        let full_array = JSON.parse(localStorage.getItem("total_array")) || [];
        let select_condition = JSON.stringify(full_array[i].select_requir);
        input_type = $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr td:last-child() input').attr('type');
        if (select_condition.match("No")) {
        }
        else if (select_condition.match("Require an answer on Every row")) {
            console.log('"Require an answer on Every row"');
            if (input_type !== undefined) {
                if (input_type == 'radio' || input_type == 'checkbox') {
                    var cod1 = 0;
                    var tr_len1 = $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr').length;
                    $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr').each(function (i, e) {
                        var count1 = 0;
                        let check_length = $(e).find('td input:checked').length;
                        if (check_length >= 1) {
                            count1 = count1 + 1;
                    ...
                    if (cod1 < tr_len1) {
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').css('display', 'block');
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').text('select atleast one answer from every row');
                    }
                    else {
                        console.log("else cod1 < tr_len1");
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').css('display', 'none');
                    }
                }
                    ...
                    let con = 0;
                    $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr td input').each(function (i, e) {
                        let check_val = $(e).val();
                        if (check_val == '') {
                            con = con + 1;
                        }
            ...
            if (input_type !== undefined) {
                if (input_type == 'radio' || input_type == 'checkbox') {
                    $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody').each(function (i, e) {
                        let count = 0;
            ...
            if (input_type !== undefined) {
                if (input_type == 'radio') {
                    ...
                    if (cod1 < tr_len1) {
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').css('display', 'block');
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').text('fill all answers');
                    }
                    else {
                        $('.table-drop .forms-sect:nth-child(' + (i + 1) + ')').find('.error_message').css('display', 'none');
                    }
                    ...
                    var tr_len2 = 0;
                    var td_len = $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr td input').length;
                    var count2 = 0;
                    $('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody').each(function (i, e) {
                        tr_len2 = tr_len2 + 1;
                        var tdinput2 = 0;
    ...
    let length = $('.table-drop .forms-sect').length;
    if (count > 0) {
        ...
        $('.preview_section .back_home').css('display', 'flex');
...
<tbody>
    ...
    table_len = $(this).parents('.container').find('.table-drop .forms-sect').length;
    ...
    for (let i = 0; i < table_len; i++) {
        ...
        let input_val = '';
        let input_attr = '';
        let input_type = $(this).parents('.container').find('.table-drop .forms-sect:nth-child(' + (i + 1) + ') tbody tr td input').attr('type');
        console.log(input_type);
        if (input_type !== undefined) {
...
$(document).on('change', '.switch', function (e) {
    let has_dropping = $('.switch .slider').css('background-color');
    console.log(typeof (has_dropping));
    let has_drop = $('.switch input:checked').length;
    console.log(typeof(has_drop));
    ...
    if (form_sect_len > 0) {
        ...
        $('.container').find('.switch input').attr('disabled', true);
    }
    if (has_dropping == 'rgb(164, 224, 31)') {
        $('.switch .slider').css('background-color', '#fff');
        $('.preview_section .back_home').css('display', 'none');

        // $.ajax({
        //     url: "index1.html", success: function (result) {
                // $(".form-sections").html(result);
                $('.table-drop .forms-sect').remove();
                $('.submit .submit-btns').remove();
                $('.submit').append('<button class="submit-btn" disabled>Submit</button>');
                $(function () {
                    $("#draggable2").draggable({
                        revert: "invalid",
                        helper: "clone",
                ...
                $('.add_element').css('display', 'flex');
                $('.sidenav').css('display', 'block');
                let forms_array = localStorage.getItem("total_array") ? JSON.parse(localStorage.getItem("total_array")) : [];
                let forms_array_len = forms_array.length;
                console.log(forms_array_len);
                for (let i = 0; i < forms_array_len; i++) {
                    ...
                    let last_th_attr = $(".table-drop #table_" + forms_sect_id + " thead tr th:nth-last-child(2)").attr('data-attr');
                    if (last_th_attr == 'undefined') {
                        $(".table-drop #table_" + forms_sect_id + " thead tr th:nth-last-child(2)").remove();
                    }
                    let last_tr_attr = $(".table-drop #table_" + forms_sect_id + " tbody tr:nth-last-child(2)").attr('data-attr');
                    if (last_tr_attr == 'undefined') {
                        $(".table-drop #table_" + forms_sect_id + " tbody tr:nth-last-child(2)").remove();
                    }
                }
                ...
                if (form_sect_len > 0) {
                    ...
                    // prev();
                    console.log('kbsakjasjk');
                }
        //     }
        // });
    }
    else {
        // $('.preview_form input:checked + .slider').css({'background-color' : '#A4E01F'});
        // $('.preview_form input:checked + .slider:before').css({'-webkit-transform': 'translateX(22px)',
        ...
        if (disabled_attr == true) {
            ...
            // $('.preview_section').append(`<div class="back_home"><img src="../images/home.png"></div>`);
            let total_array = [];
            $(this).parents('.container').find('.forms-sect').each(function (i, e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $(full_array).each(function (i, e) {
                        ...
                        $('.container .table-drop').append(yy);
                        // $('.container .table-drop .form-section').css('box-shadow', 'unset');

                        for (let i = 0; i < tr_th_length; i++) {
                            $('.table-drop #table_' + form_sect_id + ' thead tr').append("<th data-attr='" + th_div_name[i].data_attr + "'>" + "<div class='input-type' format='text name='" + th_div_name[i].name + "'>" + "" + th_div_name[i].name + "" + "</div>" + "</th>");
                        }
                        $('.table-drop #table_' + form_sect_id + ' thead tr th:first-child div').remove();
                        let last_th_attr = $('.table-drop #table_' + form_sect_id + ' thead tr th:last-child').attr('data-attr');
                        console.log(typeof (last_th_attr));
                        if (last_th_attr == 'undefined') {
                            $('.table-drop #table_' + form_sect_id + ' thead tr th:last-child').remove();
                        }
                        // if()
                        for (let i = 0; i < td_div_name.length; i++) {
                            $('.table-drop #table_' + form_sect_id + ' tbody').append(`<tr data-attr='${td_div_name[i].data_attr}'><td><div class='input-type' format='text' name='${td_div_name[i].data_attr}'>${td_div_name[i].name}</div></td></tr>`);
                        }
                        let last_tr_attr = $('.table-drop #table_' + form_sect_id + ' tbody tr:last-child').attr('data-attr');
                        if (last_tr_attr == 'undefined') {
                            $('.table-drop #table_' + form_sect_id + ' tbody tr:last-child').remove();
                        }
                        for (let i = 0; i < td_div_name.length; i++) {
                            if (input_type !== undefined) {
                                for (let j = 0; j < td_length; j++) {
                                    $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:first-child()').after(`<td><input type='${input_type}' name ='' value =''></td>`);
                                }
                            }
                            else {
                                for (let j = 0; j < select_length; j++) {
                                    $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:first-child()').after(`<td><select name="" id="">
                                    <option value="0">...</option>
                                    <option value="1">Yes</option>
                                    <option value="2">No</option>
                                    </select></td>`);
                        ...
                        for (let i = 0; i < td_div_name.length; i++) {
                            let tr_attr = $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ')').attr('data-attr');
                            for (let j = 0; j <= td_length; j++) {
                                let th_attr = $('.table-drop #table_' + form_sect_id + ' thead tr th:nth-child(' + (j + 1) + ')').attr('data-attr');
                                $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (j + 1) + ')').attr('data-attr', tr_attr + th_attr);
                            }
                        }
                        for (let i = 0; i < td_div_name.length; i++) {
                            let tr_attr = $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ')').attr('data-attr');
                            if (input_type !== undefined) {
                                if (input_type == 'radio' || input_type == 'checkbox') {
                                    for (let j = 1; j <= td_length; j++) {
                                        ...
                                        let th_attr = $('.table-drop #table_' + form_sect_id + ' thead tr th:nth-child(' + (j + 1) + ')').attr('data-attr');
                                        $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (j + 1) + ') input').attr('name', tabs_titles + tr_attr);
                                        $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (j + 1) + ') input').attr('value', th_attr);
                                    }
                                }
                                else if (input_type == 'text' || input_type == 'number') {
                                    for (let j = 1; j <= td_length; j++) {
                                ...
                                for (let j = 1; j <= select_length; j++) {
                                    let tabs_titles = 'table_' + form_sect_id;
                                    $('.table-drop #table_' + form_sect_id + ' tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (j + 1) + ') select').attr('name', tabs_titles + tr_attr);
                                }
                            }
                        }
                        $('.table-drop #table_' + form_sect_id + ' thead th:last-child').css({ 'writing-mode': 'unset' });
                    });
                    $('.container .preview_section .preview_form input:checked + .slider').css('background-color', '#A4E01F !important');
                    $('.container .preview_section .preview_form  input:checked + .slider::before').css({ 'transform': 'translateX(0px !important)' });
                    // $('.back_home').css('display', 'flex');
...
/* left side sidenav close-button*/
...
/* right side sidenav close button*/
...
$('.switch .slider').css('background-color', '#fff');
$('.preview_section .back_home').css('display', 'none');
$('.json_sec').css('display', 'none');
$('.err_message').css('display', 'none');
$('.s_message').css('display', 'none');
$('.add_element').css('display', 'block');
$('.sidenav').css('display', 'block');
$('.preview_form input').attr('checked', false);
localStorage.clear();
$("#droppable").droppable({
