        ...
        $.ajax({
            ...
            success: function (e) {
                ...
                $(".submit-btn").hover(function () {
                    $(this).css({ "background-color": "#fff", "color": "#00acc1" });
                }, function () {
                    $(this).css({ "background-color": "#00acc1", "color": "#fff" });
                });
                console.log(e);
                $('.container .forms-sect').each(function (i, e) {
                    console.log('hello');
                    let input_type = $(e).find('tbody tr:nth-child(1) td:nth-child(2) input').attr('type');
                    console.log($(e).find('tbody tr:nth-child(1) td:nth-child(2) input').attr('type'));
                    let replaceData = JSON.parse(localStorage.getItem("items")) || [];
                    if (replaceData.length === 0 || replaceData[i] == undefined) {
                        var replace = "No";
                    }
                    ...
                    console.log(replace);
                    if (input_type !== undefined) {
                        $(e).find('tbody tr td input').prop('disabled', false);

                    }
                    else {
                        $(e).find('tbody tr td select').prop('disabled', false);
                    }
...
function nextChar(c) {
    return String.fromCharCode(c.charCodeAt(0) + 1);
                ...
                //     let len = $('.container .forms-sect').length;
                //     for (let i = 0; i < len; i++) {
                //         let full_array = JSON.parse(localStorage.getItem("total_array")) || [];
                //         let select_condition = JSON.stringify(full_array[i].select_requir);
